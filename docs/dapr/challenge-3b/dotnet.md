# Challenge 3b - Pub/Sub - Programmatic

This tutorial shows how to implement Dapr programmatic pub/sub  
...
Navigate to the `before/03b-dapr-challenge-3` folder 
...
Navigate to root of your solution. Before you start coding, install the Dapr dependencies to the `PizzaKitchen` and the `PizzaDelivery` services. The `pizza-storefront` service already has the dependencies from challenge 2.
...
Now that you've published the events to the topic `orders` in the message broker, you will subscribe to the same topic in the `PizzaOrder` service:
...
Inside the `PizzaOrder` open `Program.cs` and add the `MapSubscribeHandler` registration to the *request pipeline*:
...
`MapSubscribeHandler()` in Dapr’s .NET SDK is an ASP.NET Core extension method that registers the special `/dapr/subscribe` endpoint. This endpoint tells the Dapr sidecar which topics your application subscribes to, enabling Pub/Sub integration between microservices.

The Dapr sidecar now ensures that every time a new message lands in the `orders` topic within the `pizzapubsub` pub/sub, it will be routed to the `HandleOrderUpdate` method in the `OrderController` .
...
It's time to run all four applications. If the `pizza-storefront`, `PizzaKitchen`, `PizzaDelivery`, and the `pizza-store` 
