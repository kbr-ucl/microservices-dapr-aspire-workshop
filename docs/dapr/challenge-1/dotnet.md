...
In your newly cloned repository, navigate to the `before/01-dapr-challenge-1` folder 

## Configure the state store

Navigate to the `before/shared/resources-simple` folder and create a new file called `statestore.yaml`. Add the content below to the file:
...
```bash
dapr run --app-id pizza-order --app-protocol http --app-port 8001 --dapr-http-port 3501 --resources-path ../shared/resources-simple  -- dotnet run
```
...
Alternative: Open a new terminal an navigate to the `dapr-challenge-1` folder and run the command:

```powershell
dapr run -f .
```

This command uses the `dapr.yaml` file run the system.
...
```bash
...
INFO[0000] Component loaded: pizzastatestore (state.redis/v1)  app_id=pizza-storefront instance=diagrid.local scope=dapr.runtime.processor type=log
...
```
...
If you need to reset the Redis Database, find the _CLI button at the left bottom of the screen. Click it an run the following command:
```bash
flushdb
```
